...
$('.checkbox3').change(function() {
   ...
   var myassignid=$(this).attr('name');
 
        if ($(this).prop('checked')) {
            if(confirm("Click OK! To send email to ") == true){
               //alert(myid);
                //alert(myassignid);
                checkmymail(myid,myassignid);
              alert("mail sent");
            }
            else{
...
<?php foreach($assignment as $row ){  $ttt=0; ?>
  <?php          $update=$row->udate;
        ...
        ?>
        <?php 
               foreach($userstatus as $rowstatus1){
                    ...
                    <div class="assi-body">
                        ...
                        <div class="ass_process">
                            <div class="btn-inline-wrap-letest_ass">
                               <?php  $iid=$row->aid;?>
                               <a href="<?php echo base_url();?>newadmin/editassignment/<?=$iid?>" class="view_btn btn_view-inline" alt=""> Edit </a>
                           </div>
                                       ...
                                       ?>
                                          <?php
                                                ...
                                                foreach($view as $rowview){ 
                                                 ...
                                                 <p><a href="<?php echo base_url(); ?>uploads/2_submission/<?=$currentviewfile?>"  target="_blank" style="color: white;text-decoration: none;">View</a></p>
                                                                                </div>
...
function checkmymail(myid,myassignid){
 $.ajax({
           "url": "<?php echo site_url('newadmin/mainoncheck')?>",
           "type": "POST",
           data: {name:myid,id:myassignid},
           dataType: "text",  
            cache:false,
         complete: function (response) {
           // $('#output').html(response.responseText);
...
function checkmymailss(myid,myassignid){
 $.ajax({
           "url": "<?php echo site_url('newadmin/mainoncheckss')?>",
           "type": "POST",
           data: {name:myid,id:myassignid},
           dataType: "text",  
            cache:false,
...
function getChangeStatus(a) {
    $.ajax({
           "url": "<?php echo site_url('newadmin/chat_message1234')?>",
           "type": "POST",
           data: {id:a},
           dataType: "text",  
            cache:false,
...
function getCountmessage(id1) {
    $.ajax({
           "url": "<?php echo site_url('newadmin/chat_message12')?>",
           "type": "POST",
           data: {id:id1},
           dataType: "text",  
            cache:false,
